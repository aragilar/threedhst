...
class Readfile():
    ...
    def as_pandas_dataframe(self, columns=None):
        """
        Return the catalog data formatted as a Pandas DataFrame, with 
        all of its attendendant methods for slicing and dicing
        """
        import pandas as nd
            
        cdict = {}
        for column in self.columns:
            coldata = self.__getitem__(column)
            ### Need *1 for endian formatting when reading from FITS files
            cdict[column] = coldata*1  
        
        if columns is None:
            use_columns = self.columns
        else:
            use_columns = columns
            
        df = nd.DataFrame(cdict, columns=use_columns)
        return df
